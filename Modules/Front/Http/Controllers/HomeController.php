...
//        }
        ...
        $datas = $sql->orderBy('created_at',"DESC")
                    ->where('status', 1)
                    ->get()
                    ->toArray();
    ...
    public function news(Request $request)
    {
        $list_news = News::select('id','title','alias','image_location','description','keywords','meta_title','meta_description','created_at')
                            ->paginate(9)->toArray();
        return view('front::news.index',compact('list_news'));
    }
    ...
    public function details_school ($slug = null)
    {
        ...
        $detail['relatedSchool']  = School::where('country_id', $detail->country->id)->where('id','<>', $detail->id)->get();
        // check co ho so
        $checkHoSo = false;
        if(auth()->user()){
            $user  = auth()->user();
    ...
    public function truongdaihoc(Request $request, $slug)
    {
        ...
        $this->data['obj'] = $obj->toArray();
        $schools = School::where('country_id', $obj->country_id)->get();
